...
export class NextService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    ...
    this.app = next({ dev: process.env.NODE_ENV !== 'production', quiet: true });
  ...
  handleRequest(req: Request, res: Response) {
    if (!this.handler) return res.status(200).json({ error: 'Next.js not initialized' });
    if (this.config.skip.some(p => req.path.startsWith(p))) return res.status(200).end();
    if (!this.config.silent.some(p => req.path.startsWith(p))) this.logger.log(`${req.method} ${req.path} → Next.js`);
